...
function M.close(session)
  ...
  for _, win in ipairs({ session.changes_win, session.diff_win }) do
    if win and vim.api.nvim_win_is_valid(win) then
...
function M.open(opts)
  ...
  if active_session then
    M.close(active_session)
  end

  local state = State.new({ cwd = opts.cwd or vim.fn.getcwd() })
  local is_repo, repo_err = state.git.is_repo(state.cwd)
  if not is_repo then
  ...
  local changes_buf = make_buffer("Vigit Changes", "vigit")
  local diff_buf = make_buffer("Vigit Diff", "diff")
  ...
  vim.api.nvim_create_autocmd("TabClosed", {
    ...
    callback = function()
      vim.schedule(function()
        if active_session == session and session.editor and not valid_tab(session.editor.tab) then
