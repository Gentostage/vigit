...
it("d удаляет active linked worktree после перехода на ROOT", function()
  local Fixture = require("tests.fixtures.git_repo")
  ...
  local ok, message = xpcall(function()
    ...
    local root = assert(vim.uv.fs_realpath(repo.root))
    assert_truthy(vim.wait(2000, function()
      for _, row in ipairs(picker.rows) do
        if row.path == linked_root then return row.loading == false end
      end
      return false
    end, 10))
    ...
    vim.api.nvim_win_set_cursor(picker.win, {
      assert(picker.row_by_path[linked_root]),
      0,
    })
    picker:remove()

    assert_truthy(vim.wait(3000, function()
      root_session = v2.active_session()
      return confirmations == 1
        and linked_session.closed
        and root_session ~= nil
        and root_session.root == root
        and picker.origin == root_session
        and picker.row_by_path[linked_root] == nil
    end, 10))
    assert_truthy(vim.api.nvim_win_is_valid(picker.win))
    assert_equal(picker.error, nil)
  end, debug.traceback)

  if picker and not picker.closed then picker:close() end
  if linked_session and not linked_session.closed then
    controller.dispatch(linked_session, "abandon")
  end
  if root_session and not root_session.closed then
    controller.dispatch(root_session, "abandon")
  end
