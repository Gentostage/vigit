...
it("returns stderr for a failed process", function(done)
  process.run({ "git", "rev-parse", "--verify", "does-not-exist" }, {
    ...
    assert_equal(result.error.details.args[1], "git")
    assert_equal(result.error.details.args[2], "rev-parse")
    assert_equal(result.error.details.args[3], "--verify")
    assert_equal(result.error.details.args[4], "does-not-exist")
    assert_equal(result.error.details.cwd, fixture.root)
    assert_equal(result.error.details.exit_code, 128)
    assert_truthy(type(result.error.details.stderr) == "string" and #result.error.details.stderr > 0)
    done()
  end)
end)
